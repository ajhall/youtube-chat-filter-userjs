...
// @description     Filter chats for Youtube Live
// @version         0.3
// @author          wurdn
...
(function () {
  ...
  let USER_CONFIG = {
    ...
    NGRegexpPatterns: GM_getValue("CFY_NG_REG_WORDS") || "",
  };
  ...
  const LIVE_PAGE = {
    getChatField: () => {
      ...
      if (document.getElementById("chatframe") !== null) {
        ...
        chatField = document.querySelector(
          "#items.style-scope.yt-live-chat-item-list-renderer"
        );
      }
  ...
  var findChatFieldInterval;
  const findChatField = () => {
    let FindCount = 1;
    clearInterval(findChatFieldInterval);

    const findChatField = function () {
      FindCount++;

      if (FindCount > 180) {
        log("The chat field cannot be found");
        clearInterval(findChatFieldInterval);
        FindCount = 0;
      }

      if (document.getElementById("chatframe")) {
        const chatField = LIVE_PAGE.getChatField();
        if (chatField !== null) {
          log("Found the chat field: ");
    ...
    findChatFieldInterval = setInterval(findChatField, 1000);
  };
  ...
  const initializeSettings = () => {
    ...
    if (GM_getValue("CFY_NG_REG_WORDS") !== undefined) {
      SETTINGS.NGRegexpPatterns = USER_CONFIG.NGRegexpPatterns.split(/\r\n|\n/);
    }
  };


  const createSettingsPanel = () => {
    if (document.getElementsByClassName("cfy_settings")[0] !== undefined) {
      return;
    }
...
<div class="cfy_settings">
  <div class="cfy_panel" id="cfy-setting-panel-block-or-hide">
    <div class="cfy_panel_box">
      <div>
        <span>Banned Words (case-insensitive)</span>
        <div>
      ...
      <div>
        ...
        <div>
          <textarea name="cfy_ngwords_reg" id="cfy_ngwords_reg" rows="4" style="resize: horizontal;width: 190px;">${USER_CONFIG.NGRegexpPatterns}</textarea>
        </div>
      </div>
  ...
  <div name="panelbutton" value="panelbutton" class="cfy_button" id="cfy-setting-panel-button">
    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid meet" viewBox="0 0 640 640" width="15" height="15">
      <defs>
...
`;
    ...
    const SETTINGS_PANEL_ELEMENT = document.getElementById(
      "cfy-setting-panel-block-or-hide"
    );

    document.getElementById("cfy-setting-panel-button").onclick = function () {
      if (SETTINGS.DisplaySettingPanel) {
        SETTINGS_PANEL_ELEMENT.style.visibility = "hidden";
        SETTINGS.DisplaySettingPanel = false;
      } else {
        SETTINGS_PANEL_ELEMENT.style.visibility = "visible";
    ...
    document.getElementById("cfy_input_save_button").onclick = function () {
      try {
        val = document.getElementById("cfy_ngwords").value.trim();
        USER_CONFIG.NGWords = val;
        GM_setValue("CFY_NG_WORDS", val);

        val = document.getElementById("cfy_ngwords_reg").value.trim();
        USER_CONFIG.NGRegexpPatterns = val;
    ...
    document.getElementById("cfy_reload_button").onclick = function () {
      try {
        ...
        initialize();
        ChatFieldObserver.disconnect();
        ChatFieldObserver.observe(LIVE_PAGE.getChatField(), {
          childList: true,
        });
  ...
  const createScriptCSS = () => {
    ...
    // Remove any existing CSS already added by this script
    if (ScriptCSS) {
      ScriptCSS.parentNode.removeChild(ScriptCSS);
    }

    let ScriptCSS_HTML = `
<style type="text/css" id="cfy_style">
  .cfy_panel {
  ...
  #cfy-setting-panel-button {
  }
  #cfy-setting-panel-button > svg {
    margin-right: 10px;
  }
  .toast {
    font-size: 14px;
  ...
  const handleChatFieldMutation = function (mutations) {
    mutations.forEach(function (e) {
      ...
      for (const commentNode of addedNodes) {
        if (commentNode.children.length <= 0) {
          continue;
        }

        const message = parseMessage(commentNode);

        if (containsBannedWord(message) || containsBannedRegexp(message)) {
  ...
  //--------------Comment related--------------
  const ChatFieldObserver = new MutationObserver(handleChatFieldMutation);

  // Extract the contents of the comment node
  const parseMessage = (commentNode) => {
    let messageText = "";

    // Look at the child elements of the comment
    let commentComponentNodes = Array.from(commentNode.children);

    for (const componentNode of commentComponentNodes) {
      const childId = componentNode.id;

      // Text
      if (childId === "content") {
        let message = Array.from(componentNode.children).find(
          (v) => v.id === "message"
        );
      ...
      // Superchat
      if (childId === "card") {
        let content = componentNode.children[1];
  ...
  const containsBannedWord = (chatData) => {
    if (!USER_CONFIG.NGWords || !chatData.message) {
      return false;
    }

    let target = chatData.message.toLowerCase();

    for (let i = 0; i < SETTINGS.NGWords.length; i++) {
      if (target.indexOf(SETTINGS.NGWords[i].toLowerCase()) > -1) {
        log(`Blocked: ${chatData.message} | ${SETTINGS.NGWords[i]}`);
        return true;
      }
    }
    return false;
  ...
  const containsBannedRegexp = (chatData) => {
    if (!USER_CONFIG.NGRegexpPatterns || !chatData.message) {
      return false;
    }

    let target = chatData.message.trim();

    for (let i = 0; i < SETTINGS.NGRegexpPatterns.length; i++) {
      if (SETTINGS.NGRegexpPatterns[i] === "") {
        continue;
      }

      if (RegExp(SETTINGS.NGRegexpPatterns[i]).test(target)) {
        log(
          `Blocked: ${chatData.message} | ${SETTINGS.NGRegexpPatterns[i]} (regexp)`
        );
        return true;
      }
    }
